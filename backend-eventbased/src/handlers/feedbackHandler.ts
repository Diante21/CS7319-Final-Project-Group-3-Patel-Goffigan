...
function generateFeedback(
  ...
  overallScore: number,
): { critical: string[]; improvements: string[]; strengths: string[] } {
...
export function registerFeedbackHandler(): void {
  EventBus.on(
    EVENTS.RESUME_SCORED,
    (payload: ScoredPayload & { filename?: string }) => {
      try {
        const { correlationId, overallScore, grade, sections, keywords, rawTextLength, filename } =
          payload;
        ...
        EventBus.emit(EVENTS.RESUME_FEEDBACK_GENERATED, feedbackPayload);
      } catch (err) {
